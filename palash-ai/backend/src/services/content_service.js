...
export async function getLessons() {
  const [rows] = await getDatabase().query(
    'SELECT id, grade, subject, topic, learning_outcome AS learningOutcome, hindi_instruction AS hindiInstruction, santali_translation AS santaliTranslation, is_prototype_translation AS isPrototypeTranslation FROM lessons ORDER BY id'
  );
  return rows;
}
...
export async function getLesson(id) {
  const [rows] = await getDatabase().query(
    'SELECT id, grade, subject, topic, learning_outcome AS learningOutcome, hindi_instruction AS hindiInstruction, santali_translation AS santaliTranslation, is_prototype_translation AS isPrototypeTranslation FROM lessons WHERE id = ?',
    [id]
  );
  return rows[0] ?? null;
}
